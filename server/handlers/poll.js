/*
handler: controller/API to DB e.g. CRUD
exports: so route.js could just call handle.showPolls
*/
const db = require('../models');

exports.showPolls = async (req, res, next) => {
  try {   
    // populate() == merge + filter 
    // pollSchema's user references userSchema (relational mongo)
    // populate() merges pollSchema's user w userSchema
    // then filter "merged result" to only username n id
    // i.e. polls now has user:{id:xxx, username:xxx}
...
exports.createPoll = async (req, res, next) => {
  ...
  try {
    // update DB according to schema constraint
