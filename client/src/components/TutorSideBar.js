...
class TutorSideBar extends React.Component { 
    ...
    componentWillMount(){
        console.log("debug TutorSideBar local props tree", this.props)
                                
        // after async backend call, dispatch to frontend model
        this.props.onLoad(
            agent.MC.getAll()
        );
    ...
    filterDisplayComponent(storeMCQs, filterTerm){
        // filter array of MCQs, only match filterTerm stays
        return storeMCQs.filter( MCQ => 
            MCQ.class == filterTerm)    
    }
    
    // when click side bar item, update state to only clicked items
    selectEventHandler(selected){
        this.setState({selected});   
        
        if(selected === "home"){
            // TODO
            // this.props.store == store.mc.store
    ...
    render(){
        return(
            ...
            <SideNav onSelect={this.selectEventHandler.bind(this)}>
            <SideNav.Toggle />            
